...
class MultiheadAttention(nn.Module):
    ...
    def _attn(self, q, k, v, apply_future_mask=True, padding_mask=None):
        ...
        Args:
            ...
            k: key
            v: value
            apply_future_mask:表示是否对后面进行mask，如果选择False，就代表是个双向attention；如果True，就只能看到当前之前的
            padding_mask: padding token 的位置让它变成负无穷
        Returns:
...
class TransformerModule(nn.Module):
    ...
    def forward(self, x, enc_contexts=[]):
        ...
        if self.n_segments is not None:
            ...
            padding_mask = padding_mask.float()  # checkpoint_sequential
