...
@Author: Bryan
@Comment : 定义了NLU模型的训练流程，即如何迭代数据，以及在什么情况下评测模型
"""
...
class Trainer:
    def __init__(self, args, model, tokz, train_dataset, valid_dataset,
        ...
        # 上面那个NoamOpt是为了实现一个学习率的scheduler
        # 经过实践，最好的学习率变化就是从0开始，到学习率的最大值，然后再从最大值慢慢衰减到0
        # 这个NoamOpt就实现了上述的过程
    ...
    def _eval_train(self, epoch):
        """
        在该项目中，是调用这个function来实现具体的训练过程的
        Args:
            epoch:
        Returns:
        ...
        for i, data in TQDM:
            # 下面5行是把数据加载到GPU上，non_blocking是将这个过程变成一个异步操作
            # 异步操作就是不用后面的过程等待前面的过程执行完之后才能进行
            # 模型训练的过程中，一般转移数据都是一步非常耗时间的过程，所以大多数还是要执行异步操作
            ...
            intent_logits, slot_logits = self.model(input_ids = text, attention_mask = mask,
            ...
            if (i + 1) % self.config.batch_split == 0:
                # 这是一个trick，就是积攒几个batch，然后一起计算梯度；因为可能内存吃不下特别大batch size的数据
                # 时间换空间：实际上并不能吃下那么多数据，但是可以多算几次
                # 这会让模型更加稳定
                # update weights
                self.optimizer.step()
    ...
    def train(self, start_epoch, epochs, after_epoch_funcs=[], after_step_funcs=[]):
        for epoch in range(start_epoch + 1, epochs):
            ...
            self._eval_train(epoch) # 具体的训练过程
